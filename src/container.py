...
def trans_stats(args, labels):
    ...
    '''
    tag_lists = get_tags(args.data_dir + '/ontonotes_train_no_io.conll', labels, True)
    s_o, s_i = 0., 0.
    o_o, o_i = 0., 0.
    i_o, i_i, x_y = 0., 0., 0.
...
def main():
    ...
    parser.add_argument(
        "--eval_batch_size", default=1, type=int, help="Batch size per GPU/CPU for evaluation."
    )
    ...
    # Training
    if args.do_train:
        train_dataset = read_and_load_examples(args, tokenizer, labels_train, pad_token_label_id, mode="ontonotes_train_no_io", mergeB=True)
